...
def state_to_grid(state, env):
    """
    Converts the integer state from the environment into a 3-channel grid representation
    (agent position, hole positions, goal position).
    """
    grid_size = env.unwrapped.nrow
...
class CNN_DQN(nn.Module):
    """ 
    This is the CORRECT architecture that matches your saved model file.
    It has 3 convolutional layers and 3 fully-connected layers.
    """
    def __init__(self, in_channels, action_dim, grid_size=4):
        super(CNN_DQN, self).__init__()
        # Convolutional layers
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=2),  # Output: 32x3x3
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=2),          # Output: 64x2x2
            nn.ReLU(),

            nn.Conv2d(64, 64, kernel_size=2),          # Output: 64x1x1
            nn.ReLU()
        )
...
def get_valid_actions(state, grid_size=4):
    """ 
    Returns a boolean tensor indicating which actions are valid from a given state
    to prevent the agent from trying to move off the grid.
    """
    row, col = state // grid_size, state % grid_size
    # Actions: [Left, Down, Right, Up] -> Indices [0, 1, 2, 3]
    actions = [True, True, True, True] 
...
while not done:
    ...
    with torch.no_grad(): 
        ...
        masked_q_values[0, ~valid_actions] = -1e9 
...
if reward > 0:
    ...
    print("Agent fell into a hole or the time ran out. ")
